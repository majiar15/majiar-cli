...
export default model<i${name.charAt(0).toUpperCase() + name.slice(1)}>('${
...
function contentNewProject(name: string, type: boolean = false) {
  if (type) {
    ...
    if (!fs.existsSync(`./${name}/controllers`)) {
      ...
      fs.appendFile(
        `./${name}/controllers/userController.ts`,
        contentController("user", true),
        function (err) {
          if (err) throw err;
        }
      );
    ...
    if (!fs.existsSync(`./${name}/models`)) {
      ...
      fs.appendFile(
        `./${name}/models/userModel.ts`,
        contentModel("user", true),
        function (err) {
          if (err) throw err;
        }
      );
    ...
    // add dependecis
    exec(
      `cd ./${name} && npm install express mongoose cors dotenv && npm install nodemon @types/express @types/cors @types/mongoose --save-dev`,
      (error, stdout, stderr) => {
        if (error) {
          console.log(`error: ${error.message}`);
          return;
    ...
    // add dependecis
    exec(
      `cd ./${name} && npm install express cors mongoose dotenv && npm install nodemon --save-dev`,
      (error, stdout, stderr) => {
        if (error) {
          console.log(`error: ${error.message}`);
          return;
