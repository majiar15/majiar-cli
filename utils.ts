    ...
    if (!fs.existsSync('./controllers')){
        ...
        if (type){
            fs.appendFile(`./controllers/${name}Controller.ts`, contentController(name,true), function (err) {
                if (err) throw err;
                console.log(chalk.greenBright('Controlador creado correctamente'));
    ...
    if (!fs.existsSync('./routes')){
        ...
        if (type){
            fs.appendFile(`./routes/${name}.ts`, contentRouter(name,true), function (err) {
                ...
                console.log(chalk.greenBright('Router creado correctamente'));
...
function contentNewProject(name:string, type:boolean = false){
    if(type){
        ...
        if (!fs.existsSync(`./${name}/controllers`)){
            ...
            fs.appendFile(`./${name}/controllers/userController.ts`, contentController("user",true), function (err) {
                if (err) throw err;
            });    
        ...
        if (!fs.existsSync(`./${name}/models`)){
            ...
            fs.appendFile(`./${name}/models/userModel.ts`, contentModel("user",true), function (err) {
                if (err) throw err;
            });    
        }else{
            fs.appendFile(`./${name}/models/userModel.ts`, contentModel("user",true), function (err) {
        ...
        if (!fs.existsSync(`./${name}/routes`)){
            ...
            fs.appendFile(`./${name}/routes/user.ts`, contentRouter("user",true), function (err) {
                if (err) throw err;
            });    
        }else{
            fs.appendFile(`./${name}/routes/user.ts`, contentRouter("user",true), function (err) {
        ...
        // CREATE server.js
        if (!fs.existsSync(`./${name}/server.ts`)){
            fs.appendFile(`./${name}/server.ts`, contentServer(true), function (err) {
                if (err) throw err;
            });
        }
        // add scripts package.json
        let rawdata = fs.readFileSync(`./${name}/package.json`);
        // @ts-ignore
        ...
        fs.writeFile(`./${name}/package.json`, JSON.stringify(packagejson, null, 2), (error) => {
          if (error) {
        ...
        // add tsconfig.json
        if (!fs.existsSync(`./${name}/tsconfig.json`)){
            fs.appendFile(`./${name}/tsconfig.json`, contentTsConfig(), function (err) {
                if (err) throw err;
            });
        }
        console.log(chalk.greenBright('cargando dependencias...'));
        // add dependecis
        exec(`cd ./${name} && npm install express mongoose cors dotenv && npm install nodemon @types/express @types/cors @types/mongoose --save-dev`, (error, stdout, stderr) => {
            if (error) {
                console.log(`error: ${error.message}`);
                return;
            }
            
            console.log(chalk.greenBright('Proyecto creado correctamente'));
        });
        ...
        console.log(chalk.greenBright('cargando dependencias...'));
            // add dependecis
            exec(`cd ./${name} && npm install express cors mongoose dotenv && npm install nodemon --save-dev`, (error, stdout, stderr) => {
                if (error) {
                    console.log(`error: ${error.message}`);
                    return;
                }
